...
public partial class FirmwarePage : ContentPage
{
    ...
    private async void OnApplyClicked(object? sender, EventArgs e)
    {
        ...
        bool confirmed = await DisplayAlert("Stage Pocket firmware", $"Place firmware {preview.Release.Version} at the root of {preview.PocketPath}? This does not update your cores. Afterward, safely eject the SD card, power the Pocket off, insert the card, and power it on to begin Analogue's update process.", "Stage firmware", "Cancel");
        if (!confirmed) return;
